...
public class PricingServiceServiceImpl implements PricingService {
    ...
    public PricingDto getPrice(long numOne, long numTwo) {
        PercentageDto percentage = Optional.ofNullable(percentageService.getCurrent())
                .orElseGet(() -> {
                    this.saveHistoryAsync(Constants.PRECONDITION_FAILED_MSG, Status.FAILED);
        ...
        this.saveHistoryAsync(pricing, Status.SUCCESSFUL);
        return pricing;
    }

    @Async
    public void saveHistoryAsync(Object response, Status status) {
        try {
